	...
	const handleImportGithubHandles = async () => {
		...
		if (totalAdded > 0 || totalMatched > 0) {
			...
			if (totalAdded > 0) {
				...
				if (totalMatched > 0) {
					successMessage += ` and matched ${totalMatched} existing contributor${
						totalMatched === 1 ? "" : "s"
					}`;
				}
		...
		<div className="w-full max-w-full py-6">
			...
			<Dialog open={isAddContributorsDialogOpen} onOpenChange={setIsAddContributorsDialogOpen}>
				<DialogContent className="sm:max-w-md">
					<DialogHeader>
						...
						<DialogDescription>
							Add contributors by their GitHub handles. Separate multiple handles with commas, spaces, or
							new lines.
						</DialogDescription>
					</DialogHeader>
					<div className="space-y-4 py-4">
											...
											{sublists
												...
												.map((sublist) => (
													...
													>
														<div className="flex items-center gap-2">
															<Checkbox
																checked={selectedListsForNewContributors.includes(
																	sublist.id
																)}
																onCheckedChange={(checked) =>
																	toggleListSelection(sublist.id, checked as boolean)
																}
																id={`list-${sublist.id}`}
			...
			>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>Create New List</DialogTitle>
						<DialogDescription>Create a new list with the imported contributors.</DialogDescription>
					</DialogHeader>
					<div className="space-y-4 py-4">
						<div className="space-y-2">
